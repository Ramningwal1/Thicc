...
async function CompressAggressive(messages, validationData, modelName, temperature) {
  const minReduction = Math.ceil(messages.length * 0.15);
  const maxReduction = Math.ceil(messages.length * 0.25);
  ...
  if (targetPairIndex === -1) {
    return {
      ...
      mode: 'aggressive',
      skipped: true,
      reason: `Squeeze too stubborn > need ${minReduction}-${maxReduction} messages`
    };
  }
  ...
  const summaryResult = await SummarizeMessages(deletedMessagesToSummarize, 'aggressive', modelName, temperature);
